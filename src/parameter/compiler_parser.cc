...
CompilerParameter* CompilerParser::BuildParameter(int argc, char** argv)
{
  CompilerParameter* param = Parsing(argc, argv);
  CheckParameterValid(*param);
  LOG(INFO) << "Success to build compiler parameter.";
  return param;
}

CompilerParameter* CompilerParser::Parsing(int argc, char** argv)
{
  CompilerParameter* param = new CompilerParameter();
  int opt = 0;
  int opt_index;
  const char* short_opt = "hgp";

  opt = getopt_long(argc, argv, short_opt, c_options, &opt_index);
  while (opt != -1) {
    switch (opt) {
      ...
      case 'g':
        ...
        break;
      case 'p': // pre_sched
        LOG(INFO) << "Parse -p option";
        param->SetPreScheduled(true);
        break;
      default: break;
    }
    opt = getopt_long(argc, argv, short_opt, c_options, &opt_index);
...
void CompilerParser::ParseLongOptions(int opt_index, char* exe_cmd, 
                                      CompilerParameter* param)
{
  if (strcmp(c_options[opt_index].name, "stride") == 0) {
    kStride = atoi(optarg);
    param->SetStride(kStride);
  } else 
  if (strcmp(c_options[opt_index].name, "iw") == 0) {
    param->SetIw(atoi(optarg));
  } else 
  if (strcmp(c_options[opt_index].name, "ih") == 0) {
    param->SetIh(atoi(optarg));
  } else 
  if (strcmp(c_options[opt_index].name, "ic") == 0) {
    param->SetIc(atoi(optarg));
  } else
  if (strcmp(c_options[opt_index].name, "pw") == 0) {
    kPadding = atoi(optarg);
    param->SetPw(kPadding);
  } else 
  if (strcmp(c_options[opt_index].name, "ph") == 0) {
    param->SetPh(atoi(optarg));
  } else
  if (strcmp(c_options[opt_index].name, "kw") == 0) {
    kFilter_len = atoi(optarg);
    param->SetKw(kFilter_len);
  } else 
  if (strcmp(c_options[opt_index].name, "kh") == 0) {
    kFilter_len = atoi(optarg);
    param->SetKh(kFilter_len);
  } else 
  if (strcmp(c_options[opt_index].name, "oc") == 0) {
    param->SetOc(atoi(optarg));
  } else 
  if (strcmp(c_options[opt_index].name, "mac-cycles") == 0) {
    param->SetMacCycles(atoi(optarg));
  } else 
  if (strcmp(c_options[opt_index].name, "frequency") == 0) {
    param->SetFrequency(atof(optarg));
  } else 
  if (strcmp(c_options[opt_index].name, "bandwidth") == 0) {
    param->SetBandwidth(atof(optarg));
  } else 
  if (strcmp(c_options[opt_index].name, "input-mem-size") == 0) {
    param->SetInputMemSize(atof(optarg) * 1024.0);
  } else
  if (strcmp(c_options[opt_index].name, "weight-mem-size") == 0) {
    param->SetWeightMemSize(atof(optarg) * 1024.0);
  } else
  if (strcmp(c_options[opt_index].name, "output-mem-size") == 0) {
    param->SetOutputMemSize(atof(optarg) * 1024.0);
  } else 
  if (strcmp(c_options[opt_index].name, "pe-dim") == 0) {
    param->SetPeDim(Matrix(optarg, strlen(optarg)));
  } else 
  if (strcmp(c_options[opt_index].name, "pe-structure") == 0) {
    param->SetPeStructure(Matrix(optarg, strlen(optarg)));
  } else 
  if (strcmp(c_options[opt_index].name, "code-path") == 0) {
    param->SetCodeFile(optarg);
  } else 
  if (strcmp(c_options[opt_index].name, "gaia-path") == 0) {
    param->SetGaiaFile(optarg);
  } else
  if (strcmp(c_options[opt_index].name, "latency-path") == 0) {
    param->SetLatencyFile(optarg);
  } else 
  if (strcmp(c_options[opt_index].name, "timestamp-path") == 0) {
    param->SetTimestampFile(optarg);
  } else 
  if (strcmp(c_options[opt_index].name, "tiling-dump") == 0) {
    param->SetTilingDumpFile(optarg);
  } else 
  if (strcmp(c_options[opt_index].name, "loop-seq-dump") == 0) {
    param->SetLoopSequenceDumpFile(optarg);
  } else 
  if (strcmp(c_options[opt_index].name, "layer") == 0) {
    param->SetLayerName(optarg);
  } else 
  if (strcmp(c_options[opt_index].name, "help") == 0) {
    PrintHelp(exe_cmd);
    exit(EXIT_SUCCESS);
  } else {
    LOG(ERROR) << "Invalid argument: " << optarg;
  }
  LOG(INFO) << "Parse --" << c_options[opt_index].name << " : " << optarg;
}

void CompilerParser::CheckParameterValid(const CompilerParameter& param) const
{
  ...
  CHECK(strcmp(param.GetCodeFile(), "") != 0) << "Code file is empty.";
  CHECK(strcmp(param.GetGaiaFile(), "") != 0) << "Gaia IR file path is empty.";
  CHECK(strcmp(param.GetLatencyFile(), "") != 0) <<"Latency file is empty.";
  CHECK(strcmp(param.GetTimestampFile(),"")!=0) << "Timestamp file is empty.";
...
void CompilerParser::PrintHelp(char* exe_cmd) const
{
  cout    << "e-PlaNNer Compiler."
  ...
  << endl << "--code-path=<path>      Generated code path"
  << endl << "--gaia-path=<path>      Generated Gaia IR path"
  << endl << "--latency-path=<path>   Latency file path"
  << endl << "--timestamp-path=<path> Timestamp JSON record file path"
